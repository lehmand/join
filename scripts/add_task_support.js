  ...
  for (let i = 0; i < subtasksList.length; i++) {
    ...
    lists.innerHTML += /*html*/ `   
                 ...
                 <div class="add-task-subtasks-icons">
                    <img class="add-task-edit" src="./assets/icons/edit_dark.svg"  onclick="editTaskSubtasksList(${i})" >
                     <div class="add-tasks-border"></div>
                    <img  class="add-task-trash" src="./assets/icons/basket.svg" onclick="removeFromAddTaskSubtasksList(${i})">
                 </div>
               </li>
                 `;
  }
 ...
 */
function removeFromAddTaskSubtasksList(i) {
  subtasksList.splice(i, 1);
  globalSubtasks.splice(i, 1);
  addTaskSubtasksList();
}

function editTaskSubtasksList(param) {
  ...
  for (let i = 0; i < subtasksList.length; i++) {
    ...
    if (i === param) {
      ...
      trashIcon.setAttribute("onclick", `removeFromAddTaskSubtasksList(${i})`);
...
function confirmTaskSubtasksList(i, event) {
  ...
  const element = document.getElementById("add-task-subtasks-input-edit").value;
  subtasksList.splice(i, 1, element);
  addTaskSubtasksList();
}
